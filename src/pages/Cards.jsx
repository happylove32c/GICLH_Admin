...
const Cards = () => {
  ...
  return (
    <div className="p-5">
      {showModal && (
        <div className="fixed inset-0 bg-gray-900 p-4 bg-opacity-75 flex justify-center items-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {events.map((event) => (
              <div key={event.id} className="p-4 border rounded-lg shadow-md">
                ...
                <button onClick={() => handleEdit(event)} className="mt-3 bg-blue-500 text-white px-3 py-1 rounded-md mr-2">
                  Edit
                </button>
          ...
          {showEditEventModal && (
            <div className="fixed inset-0 p-6 bg-gray-900 bg-opacity-75 flex justify-center items-center">
              <div className="bg-white p-6 rounded-lg shadow-lg w-96 text-center">
                ...
                {Object.keys(editingEvent).map((key) => (
                  <input
                    key={key}
                    type="text"
                ...
                <button onClick={handleUpdateEvent} className="w-full bg-green-600 text-white py-2 rounded hover:bg-green-700">
                  Update Event
                </button>
                <button onClick={() => setShowEditEventModal(false)} className="w-full bg-gray-400 text-white py-2 rounded mt-2">
                  Cancel
                </button>
              </div>
            </div>
